...
func GetPost(uuid uuid.UUID) (types.PostWithUsername, error) {
    ...
    if row.Err() != nil {
        ...
        var post types.PostWithUsername 
        row.Scan(&post.Id, &post.UserId, &post.Content, &post.CreatedAt, &post.UpdatedAt, &post.Username)
        return post, nil
    }
}
...
func UpdatePost(post types.Post) error {
    _, err := database.DB.Exec("UPDATE posts SET content = $1, updated_at = $2", post.Content, time.Now())
    return err
}
